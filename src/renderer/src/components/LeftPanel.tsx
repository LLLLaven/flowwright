...
export default function LeftPanel({ view, onClose }: LeftPanelProps) {
  return (
    <div className="flex w-[264px] shrink-0 flex-col border-r border-[#1e293b] bg-[#0f172a]">
...
function NodeLibrary() {
  const kinds = [
    { key: "agent", icon: "🤖", label: "Agent", desc: "LLM 推理节点", accent: "#6366f1" },
    { key: "human", icon: "👤", label: "Human Review", desc: "人工审核节点", accent: "#f59e0b" },
    { key: "condition", icon: "🔀", label: "Condition", desc: "条件路由节点", accent: "#8b5cf6" },
    { key: "rag", icon: "📚", label: "RAG Retrieve", desc: "向量检索节点", accent: "#10b981" },
  ];
  return (
    <div className="flex flex-col gap-2">
      ...
      {kinds.map((k) => (
        <div key={k.key}
          ...
          <div className="flex size-9 shrink-0 items-center justify-center rounded-lg text-[18px]"
            style={{ background: `${k.accent}1a` }}>
            {k.icon}
          </div>
          <div className="min-w-0 flex-1">
            <div className="text-[13px] text-slate-100">{k.label}</div>
...
function RagList() {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      {docs.map((d) => (
        <div key={d} className="flex items-center gap-2.5 rounded-lg px-3 py-2 text-[13px] text-slate-300 hover:bg-slate-700/50">
